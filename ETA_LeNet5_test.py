...
class regression_cnn(object):
    ...
    def creat(self):
        with tf.variable_scope("cnn") as scope1:
            ...
            # 第五层：全连接层
            fc1_weights = tf.get_variable("fc1_weights", [4 * 4 * 64, 1024],
                                          initializer=tf.truncated_normal_initializer(
            ...
            # 第六层：全连接层
            fc2_weights = tf.get_variable("fc2_weights", [1024, 1], initializer=tf.truncated_normal_initializer(
            ...
            with tf.variable_scope("readTrainData")as testData_scope:
                dataset = tf.data.TFRecordDataset(filenames=[self.testDataSet])
            ...
            model_path = self.outFile
            with open("testError.log","w") as o:
                with tf.Session() as sess:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            print(image[-1, 117])
                            if float(image[-1,117])<=0.015:
                                continue
                            label = label.reshape(-1, 1)
                            predictVal=sess.run(prediction,{xs: image, ys: label,keep_prob: 1.0} )
                            # print(predictVal*68000)
                            print(abs(label-predictVal)/label)

                            o.write(
                                str(abs(label - predictVal) / label).replace("[[", "").replace("]]", "") + "," + str(
                            ...
                            # o.write(",")
                            # o.write(str(image[-1, 117]*188235+2000).replace("[[","").replace("]]",""))
                            # o.write(",")
                            # o.write(str(label*68000).replace("[[","").replace("]]",""))
                            o.write("\n")
                            ...
                            # sess.run(train_step, feed_dict={xs: image, ys: label, keep_prob: 1.0})
                            if i % 50 == 0:
...
obj=regression_cnn("eta_test_137.tfrecords","eta_train_137.tfrecords","model_LeNet5/model.ckpt")
